...
class Database:
    ...
    async def find_user_by_email(self, email: str) -> Optional[dict]:
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM users WHERE email = ?", (email.strip().lower(),)) as cursor:
    ...
    async def find_user_by_link_code(self, code: str) -> Optional[dict]:
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM users WHERE link_code = ?", (code,)) as cursor:
                row = await cursor.fetchone()
                return dict(row) if row else None

    async def set_link_code(self, chat_id: int, code: str) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE users SET link_code = ? WHERE chat_id = ?", (code, chat_id))
            await db.commit()

    async def clear_link_code(self, chat_id: int) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE users SET link_code = NULL WHERE chat_id = ?", (chat_id,))
            await db.commit()
    ...
    async def link_account(self, old_chat_id: int, new_chat_id: int) -> bool:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM users WHERE chat_id = ?", (old_chat_id,)) as cursor:
                old_user = await cursor.fetchone()
            if not old_user:
                return False

            # Ensure target user exists
            async with db.execute("SELECT 1 FROM users WHERE chat_id = ?", (new_chat_id,)) as cursor:
                exists = await cursor.fetchone()
            if not exists:
                await db.execute(
                    "INSERT INTO users (chat_id, is_active) VALUES (?, 1)", (new_chat_id,)
                )

            # Copy subscriptions (skip duplicates)
            async with db.execute(
                "SELECT type, value FROM subscriptions WHERE chat_id = ?", (old_chat_id,)
            ) as cursor:
                for row in await cursor.fetchall():
                    await db.execute(
                        "INSERT OR IGNORE INTO subscriptions (chat_id, type, value) VALUES (?, ?, ?)",
                        (new_chat_id, row[0], row[1]),
                    )

            # Delete old subscriptions
            await db.execute("DELETE FROM subscriptions WHERE chat_id = ?", (old_chat_id,))
            ...
            await db.execute("""
                UPDATE users SET
                    ...
                    is_active = 1
                WHERE chat_id = ?
            """, (old_user['email'] or '', old_user['telegram'] or '', old_user['password_hash'] or '', new_chat_id))
