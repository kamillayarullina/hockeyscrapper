# Customer Meeting Summary

## Date and participants

14.06.2026
- Kamilla Iarullina
- Gleb Shamiev
- Samir Shakirov
- Bulat Bulatov

![Prototype of main screen](images/Mainscreen.png)
![Prototype of main screen](images/Mainscreen2.png)
![Prototype of main screen](images/Mainscreen3.png)
![Prototype of registration screen](images/Register.png)
![Prototype of login screen](images/Login.png)
![Prototype of profile screen](images/Profileuser.png)
![Prototype of subscriptions screen](images/Subscriptions.png)

## Discussion points

- Prioritization of User Stories
- Current product
- Product design

## Decisions

Make github repository public

## Artifacts demonstrated

- Website
- Telegram bot
- Figma

## Action Points

- Add team logos to website
- Fix oversized input fields on registration page
- Implement email notifications
- Implement password recovery

## Risks Identified

- Parser could be blocked by CAPTCHA due to frequent requests
- Testing on intended websites is currently not available

## Customer Feedback

- Make font in input fields bigger
- add team logos

## Customer Approvals Obtained

| Item | Status |
|------|--------|
| Public repository (GitHub) | ✅ Approved|
| Recording of meeting | ✅ Approved |
| User stories | ✅ Agreed |
| Demo of current functionality | ✅ Accepted |

## Resulting Changes
| Story / Artifact | Change |
|-----------------|--------|
| Team Logos | Add team logos |
| Registration Form | Resize input fields |
| Email Notification | Add notification via email |

## Links to evidence
- Figma mockups https://www.figma.com/proto/QA9kxzUyxzBCnEycOYvinv/HockeyScrapper-Prototype?node-id=3-163&p=f&t=cW9lcLU0V8yCkDc5-1&scaling=scale-down&content-scaling=fixed&page-id=0%3A1&starting-point-node-id=3%3A163&show-proto-sidebar=1
- Interview recording https://drive.google.com/file/d/1jd-_YTGIgSYCszZ7eRl1LAxsamZqYGi3/view?usp=sharing
- Transcript source [`reports/week2/customer-meeting-transcript.md`](customer-meeting-transcript.md)
